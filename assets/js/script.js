...
class Shape {
  ...
  render() {
    ...
    endShape(CLOSE);
    if (this.showPoints) {
      let points = [...this.points, this.center].sort(
        (a, b) => a.depth - b.depth
      );
...
let Box = (anchor, w, l, h, style = {}) => {
  let verts = [anchor, anchor.clone().plus(new Vec3(w, 0, 0))];
...
let Tree = (x, y, z, h) => {
  ...
  let col = color(random(0.23, 0.3), random(0.7, 0.8), random(0.3, 0.5));
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    shapes.push(
      new Shape(
        [points[i], points[(i + 1) % points.length], base],
...
let init = () => {
  ...
  for (let i = 0; i < rows; i++) {
    ...
    let x = (i / rows - 0.5) * gridSize;
    grid.push([]);
    for (let j = 0; j < cols; j++) {
      if (window.CP.shouldStopExecution(2)) break;
      let y = (j / cols - 0.5) * gridSize;
  ...
  let col1 = color(0.25, 0.8, 0.8);
  let col2 = color(0.5, 0.1, 1);
  ...
  let tris = (p1, p2, p3) => {
    ...
    let l = s.normal.dot(light) * 0.5 + 0.5;
    let hue = lerp(0.25, 0.5, amt);
    let sat = lerp(0.8, 0, amt);
    let bal = lerp(0.8, 1, amt) * (1 - l);
